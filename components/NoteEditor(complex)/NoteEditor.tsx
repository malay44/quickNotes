/*
 * This component is Still WIP
 * It was an effort towards not using depreciated browser API
 * Due to time constraints, it is not fully functional yet and some features are missing
 * For now, this app is not using this component instead using a depreciated browser API
 * Some Bugs to solve:
 * - after pressing enter, block is spammed with lots of spans
 * - on deleting a block, error is thrown some time
 */
...
import { handleKeyDown } from "./helper"; // Make sure to create this file if it doesn't exist

interface ComplexNoteEditorProps {
...
const ComplexNoteEditor: React.FC<ComplexNoteEditorProps> = ({
  ...
  useEffect(() => {
    if (selectedBlockId) {
      const block = blockRefs.current[selectedBlockId];
      if (block) {
        block.focus();
      }
    }
  ...
  const handleKeyDownWrapper = (e: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    if (blockId) {
      handleKeyDown(e, blockId, blocks, setBlocks, createNewBlock);
    }
  };
  ...
  return (
    <div className="p-2 h-full">
      <div className="flex gap-2 mb-2">
        <Button variant="outline" onClick={() => applyFormatting("b")}>
          B
        </Button>
      ...
      <div className="min-h-[200px] border p-2 rounded-md h-full">
        {blocks.map((block) => (
          <div
            ...
            onKeyDown={handleKeyDownWrapper} // Add this line
            style={{
...
export default ComplexNoteEditor;
